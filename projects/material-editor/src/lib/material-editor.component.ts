...
import { MaterialEditorConfig } from './config';

@Component({
  selector: 'lib-material-editor',
  templateUrl: './material-editor.component.html',
  styleUrls: ['./material-editor.component.scss'],
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => MaterialEditorComponent),
      multi: true
    }
  ]
})
export class MaterialEditorComponent implements OnInit, ControlValueAccessor, AfterViewInit {
  ...
  disabled = false;
  @Input() id = '';
  @Input() config: MaterialEditorConfig = MaterialEditorConfig;
  @Input() placeholder = '';
  ...
  @ViewChild('editorToolbar', { static: false } ) editorToolbar: MaterialEditorToolbarComponent;
  ...
  ngAfterViewInit() {
    ...
    this.config.toolbarPosition = this.config.toolbarPosition ? this.config.toolbarPosition : MaterialEditorConfig.toolbarPosition;
    if (this.config.showToolbar !== undefined) {
      this.editorToolbar.showToolbar = this.config.showToolbar;
    }
    this.editorToolbar.fonts = this.config.fonts ? this.config.fonts : MaterialEditorConfig.fonts;
  ...
  onTextAreaBlur(event: FocusEvent) {
    ...
    if (event.relatedTarget != null && (event.relatedTarget as HTMLElement).parentElement.className !== 'material-editor-toolbar-set') {
    this.blur.emit('blur');
    }
  }
